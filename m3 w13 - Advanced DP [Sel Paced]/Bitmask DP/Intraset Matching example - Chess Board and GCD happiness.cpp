...
// happiness of assigning i,j players to kth board is k*abs(A[i]-A[j])*gcd(A[i],A[j])
...
// }
// int temp=ans;
                    ans = max(ans, (board+1)*abs(a[i]-a[j])*gcd(a[i],a[j])+rec(board+1, mask|(one<<i)|(one<<j), a, dp));
// if(ans != temp){
//     ii = i; jj = j;
// cout<<"gesfgesf"<<endl;
// }
